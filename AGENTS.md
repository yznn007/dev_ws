...
- 仓库根目录 `config/` 存放 DNN 模型配置，供 RDK X5 上 Hobot AI 推理节点使用（`origincar_bringup` 的摄像头/Websocket 相关启动依赖）
...
- `wheeltec_udev.sh`（及同目录二进制 `wheeltec_udev`）会写 `/etc/udev/rules.d/` 并重启 udev，属于宿主机级修改；未明确要求不要执行。
