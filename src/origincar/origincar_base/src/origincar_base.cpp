...
using namespace std;
static origincar_base* g_robot_node = nullptr;
...
//    }
  ...
  // 定义一个固定的频率（比如 50Hz）
    ...
    while(rclcpp::ok()) {
        ...
        // 1. 无论是否拿到传感器数据，都要处理 ROS 消息回调
        rclcpp::spin_some(this->get_node_base_interface());

        if (Get_Sensor_Data()) {
            // 2. 积分计算（这里建议对 Sampling_Time 做个异常保护）
            if (Sampling_Time > 0 && Sampling_Time < 0.2) { 
                Robot_Pos.X += 1.03 * (Robot_Vel.X * cos(Robot_Pos.Z) - Robot_Vel.Y * sin(Robot_Pos.Z)) * Sampling_Time;
                Robot_Pos.Y += 1.125 * (Robot_Vel.X * sin(Robot_Pos.Z) + Robot_Vel.Y * cos(Robot_Pos.Z)) * Sampling_Time;
                Robot_Pos.Z += Robot_Vel.Z * Sampling_Time;
            }
            ...
            // 只有成功处理了数据，才更新 last_time
            last_time = current_time;
        }

        // 3. 必须让线程休眠，否则 CPU 会烧到 100%
        loop_rate.sleep(); 
    }
}
...
void sigintHandler(int sig)
{
    //单独硬编码的串口，如果串口出问题大概在这里
    std::string usart_port_name = "/dev/ttyCH343USB0";
